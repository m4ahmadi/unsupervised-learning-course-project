...
def getRandomInitial(n_features, n_cluster):
  centroids = np.zeros((n_cluster, n_features), dtype=Decimal)
  random.seed()
  for k in xrange(n_cluster):
    coeff = 0
    for index in xrange(n_features):
      temp  = random.randint(25, 75)
      centroids[k, index] = dtype=Decimal(temp/20)
      #coeff += temp
    #centroids[k, :] /= coeff
  return centroids

def BrnouliEM(datax, n_cluster, n_init, max_iterations):
  ...
  old_miu = np.zeros((n_cluster, n_feature), dtype=Decimal)
  z = np.zeros((n_data, n_cluster), dtype=Decimal)
  old_pi = np.zeros((n_cluster), dtype=Decimal)
  pi = np.zeros((n_cluster), dtype=Decimal)
  ...
  while init < n_init:
     ...
     for k in xrange(n_cluster):
       pi[k] = Decimal(1/n_cluster)
     #print(pi)
     if best_likelihood < likelihood:
       best_predict = predict
       best_likelihood = likelihood
       
     likelihood = Decimal(-10000000.0)
     old_likelihood = Decimal(-100000.0)
     t = 0.0
 	...
 	t +=1.0
    	...
    	#E-Step
    	temp = np.zeros((2,1), dtype=Decimal)
    	for n in xrange(n_data):
          ...
          for k in xrange(n_cluster):
            ...
            for i in xrange(n_feature):
              if datax[n, i]==1:
                  tmp = Decimal(float(old_miu[k, i]))
              else:
                  tmp = Decimal(float(5-old_miu[k, i]))
	...
	likelihood = Decimal(0.0)
        for n in xrange(n_data):
          predict[n] = np.argmax(z[n,:])
          for k in xrange(n_cluster):
            temp = Decimal(log(old_pi[k]))
            for i in xrange(n_feature):
              if(datax[n,i]==1):
                temp += Decimal(float(old_miu[k, i]/5)).ln()
              else:
                temp += Decimal(float(5 - old_miu[k, i])/5).ln() 
            temp *= z[n, k]
          likelihood  += temp
        ...
        #M-Step	  
    	pi = z.sum(axis=0)
    	miu = np.zeros((n_cluster, n_feature), dtype=Decimal)
        #miu = decimal.Decimal(z.transpose()) * datax
        for n in xrange(n_data):
          for k in xrange(n_cluster):
            for i in xrange(n_feature):
              miu[k,i] += Decimal(float(float(z[n,k]) * datax[n,i]))
        for k in xrange(n_cluster):
          if pi[k]!=0:
            miu[k,:] /= pi[k]*Decimal(float(0.2))
          pi[k] /= n_data
        print(pi)
...
t0 = time()
estimator = BrnouliEM(reduced_data_bw[1:10,:], n_cluster, 4, 10)
bench("Bernouli EM",(time() - t0), labels[1:10], estimator, n_cluster, n_data, n_feature)
